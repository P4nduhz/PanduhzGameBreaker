...
namespace PanduhzGameBreaker
{
    ...
    public class PanduhzGameBreakerBaseMod : BaseUnityPlugin
    {
        ...
        private void SetBindings()
        {
            ...
            //-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            //floats
            jumpHack = Config.Bind("Hacks", "Player - Jump Force", 15f, new ConfigDescription("Base jump force for player", new AcceptableValueRange<float>(5.0f, 500.0f)));//20 is a pretty good jump force
        ...
        internal static void FlyHackFunction(ref float ___movementSpeed)//default 0.5
        {
            ...
            if (flyHack)
            {
                float Speed = ___movementSpeed * Time.deltaTime;
                ...
                {
                    PanduhzGameBreakerBaseMod.Instance.Player.playerRigidbody.transform.position += PanduhzGameBreakerBaseMod.Instance.Player.playerGlobalHead.transform.forward * Speed;
                }
                if (UnityInput.Current.GetKey(KeyCode.A))
                {
                    PanduhzGameBreakerBaseMod.Instance.Player.playerRigidbody.transform.position += PanduhzGameBreakerBaseMod.Instance.Player.playerGlobalHead.transform.right * -Speed;
                }
                ...
                {
                    PanduhzGameBreakerBaseMod.Instance.Player.playerRigidbody.transform.position += Vector3.up * Speed;
                }
                if (UnityInput.Current.GetKey(KeyCode.LeftControl))
                {
                    PanduhzGameBreakerBaseMod.Instance.Player.playerRigidbody.transform.position += PanduhzGameBreakerBaseMod.Instance.Player.playerGlobalHead.transform.up * -Speed;
                }
        ...
        {
            var = RoundManager.Instance.playersManager.SetShipDoorsClosed(closed: true);
        }
